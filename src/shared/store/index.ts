		...
		(set, get) => ({
			...
			fetchTimezones: async () => {
				try {
					...
					if (typeof window !== "undefined") {
						...
						if (
							savedTimezone &&
							savedTimezone !== "Select your timezone"
						) {
							set({
			...
			updateUser: async (updateData: UpdateUserParams) => {
				try {
					...
					if (updateData.notif_deadline !== undefined) {
						dataToSend.notif_deadline = updateData.notif_deadline
							? "1"
							: "0";
					}

					if (updateData.notif_articles !== undefined) {
						dataToSend.notif_articles = updateData.notif_articles
							? "1"
							: "0";
					}

					if (updateData.notif_guides !== undefined) {
						dataToSend.notif_guides = updateData.notif_guides
							? "1"
							: "0";
					}

					if (updateData.notif_change !== undefined) {
						dataToSend.notif_change = updateData.notif_change
							? "1"
							: "0";
					}
			...
			payWithYookassa: async (amount: number) => {
				try {
					...
					set({
						paymentRedirectUrl: (
							response.data as { redirect_url: string }
						).redirect_url,
						error: null,
					});
				} catch (error) {
			...
			fetchSubscriptions: async () => {
				try {
					...
					const response =
						await axiosInstance.get<SubscriptionsResponse>(
				...
				try {
					...
					if (params) {
						...
						if (params.favorites !== undefined)
							queryParams.append(
								"favorites",
								params.favorites.toString()
							);
						if (params.sorting)
							queryParams.append(
								"sorting",
								params.sorting.toString()
							);
					}
					...
					const response = await axiosInstance.get<GuidesResponse>(
						url
					);
			...
			toggleTaskComplete: async (taskId: number) => {
				try {
					...
					if (currentGuideDetails) {
						const updatedTasks = currentGuideDetails.tasks.map(
							(task) => {
								if (task.id === taskId) {
									return {
										...task,
						...
						const isCompleting =
							currentGuideDetails.tasks.find(
								(task) => task.id === taskId
							)?.completed === 0;
						...
						set({
							guideDetails: {
								...
								competed_tasks_count: isCompleting
									? currentGuideDetails.competed_tasks_count +
									  1
									: currentGuideDetails.competed_tasks_count -
									  1,
							},
					...
					if (currentGuideDetails) {
						const revertedTasks = currentGuideDetails.tasks.map(
							(task) => {
								if (task.id === taskId) {
									return {
										...task,
			...
			fetchBlogArticles: async (params?: BlogArticlesParams) => {
				try {
					...
					const response =
						await axiosInstance.get<BlogArticlesResponse>(url);
			...
			fetchBlogArticleDetails: async (idOrSlug: string | number) => {
				try {
					...
					const response =
						await axiosInstance.get<BlogArticleDetails>(
					...
					console.error(
						"Error fetching blog article details:",
						error
					);
			...
			toggleRead: async (articleId: number) => {
				try {
					...
					if (
						...
						currentArticleDetails.id === articleId
					) {
						set({
							blogArticleDetails: {
								...currentArticleDetails,
			...
			fetchProductCategories: async () => {
				try {
					...
					const response =
						await axiosInstance.get<ProductCategoryResponse>(
			...
			createOrder: async (params: OrderCreateParams) => {
				try {
					...
					const response =
						await axiosInstance.post<OrderCreateResponse>(
							"/api/orders/store",
							params,
							{
								withCredentials: true,
			...
			fetchSubaccounts: async (page?: number) => {
				try {
					...
					const response =
						await axiosInstance.get<SubaccountsResponse>(url);
			...
			login: async (data: LoginParams) => {
				...
				try {
					...
					try {
						...
						console.warn(
							"Error fetching user after login:",
							userError
						);
						set({ sessionVerified: false, user: null });
						updateAxiosToken(null);
			...
			fetchNotifications: async (page?: number) => {
				try {
					...
					const response =
						await axiosInstance.get<NotificationsResponse>(url);
